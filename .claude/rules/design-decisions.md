...
| **Suítes visuais fora do agregado local** | KI-3: exigem Chromium; execução canônica é o job `visual` do CI (em calibração) e o rito manual do proprietário. O `env-doctor` reporta a ausência — isso é o desenho, não uma lacuna. Decisão original — registrada aqui e superada — calibração fechada na Onda 4 em 6 rodadas: playwright 67/37 canônico, P50 27/27, P52 44/44 com âncora imutável, campanha p52 72/72 por trigger (`known_issues.json → _meta`; `verify.yml:47`). A demanda 016 foi além de calibrar: tornou `visual` check **obrigatório** para o merge em `develop`, ao lado de `verify` e do novo `fecho` (P2, `.claude/verify/branch_protection.json → checks_obrigatorios`), auditado pela seção `branch-protection` do `compliance-audit.sh` (`D016-PROT1`). A execução canônica continua sendo o CI e as suítes continuam exigindo Chromium — isso não mudou; o que mudou é que "em calibração" deixou de ser verdade e a cobrança do merge deixou de ser só rito manual: vive na proteção de branch, dado auditável (ADR `docs/adr/0001-cobranca-no-merge-fora-do-repositorio.md`). |
...
| **A lista "Pode fazer sentido — após validação" convive com a visão por solução** | Desde 2026-09-22 (`EA-68`) o mesmo produto aparece em dois lugares da mesma tela: como **card** na visão por solução e como **item** na lista secundária. A redundância é **real e deliberada**. Refutada a demanda **020-lista-secundaria-redundante** no portão da Fase 0 — fonte: decisão do proprietário no chat, 2026-09-24, *"Não remover a lista, fecha a 020 como refutada"*. A razão está medida, não suposta: remover o título congelado derruba **cinco gates da demanda 010** (`D010-ARB3`, `INV7`, `CARD1`, `CARD2`, `CARD3`), todos por uma raiz só — as fixtures `D010-F3` **declaram** aquele título como presente. Não é gate de apresentação que se reancora: é o **oráculo declarado de outra demanda**, e a 010 existe para impedir que conteúdo da Camada 1 desapareça sem substituto. Do outro lado: legibilidade, sem nenhuma afirmação falsa. **Custo estrutural contra benefício estético.** Registro completo em `specs/020-lista-secundaria-redundante/relatorio-final.md`, com o erro de estimativa que precedeu a medição (*"ciclo de uma hora"*, errado por uma ordem de grandeza). **Não reportar como defeito de apresentação nem como duplicação a corrigir.** |
