...
| **Evidência binária (~103 MB) versionada** | Herança das fases 5.0–5.2, pinada pelos manifestos de fase. Decisão original — permanecer até a migração desenhada (Onda 4/R11) — registrada aqui e superada em execução pela demanda 007 (`specs/007-migracao-evidencia/`): os 4 acervos (`evidence_p50`/`evidence_p51`/`evidence_p52`/`evidence_unset`, 406 arquivos) migram para Releases nominais (`evidence-p50`/`evidence-p51`/`evidence-p52`/`evidence-unset`) em `oflavioc/quickscan-secops`, com verificabilidade preservada por manifesto-ponte pinado (`.claude/verify/evidence_bridge.json`) e pelo gate `evidence-bridge`. `evidence_v322` (gate V322-DOC3) e os 3 ZIPs da raiz (gates S64/S74/S113, migração de escopo posterior) ficam versionados. Trilha: decisão original permanece citável acima; a consumação plena (publicação + desindexação + repin) é registrada no relatório final da demanda 007 (T014) e não é afirmada aqui. Evidência NOVA segue R11. |
