...
out = HERE/"ui_icons_v32.js"
if "--check" in sys.argv:                                     # [Onda-0] verificação sem escrita (pipeline)
    cur = out.read_bytes() if out.exists() else b""
    if cur != js.encode("utf-8"):
        print("ui_icons_v32.js DIVERGE do gerador — rode sem --check para regravar conscientemente")
        sys.exit(1)
    print("ui_icons_v32.js: check OK ·", len(assets), "assets · sha256:", hashlib.sha256(js.encode()).hexdigest()[:16])
else:
    with open(out, "w", encoding="utf-8", newline="\n") as f:  # [Onda-0] LF por construção em qualquer SO
        f.write(js)
