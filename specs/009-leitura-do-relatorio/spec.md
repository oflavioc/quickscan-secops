...
| **Por que estava errado** | Entre a string canônica e o que a tela mostra existe uma transformação **pública e anterior a esta demanda**: o mapa de apresentação da 5.2 (`ui_p52_workspace_v32.js:128`), exposto por `__P52.copyMap()` / `__P52.applyCopy()`. Ele troca, entre outros pares, `"Mandato e objetivos"` → `"Direcionamento e objetivos"` e `"mandato"` → `"direcionamento"`. Comparar sem aplicá-lo mediria uma divergência da **5.2**, não o efeito da 009 |
...
| **O que passa a valer** | A comparação é byte a byte contra `__P52.applyCopy(paragraphs[k])`, com o mapa lido do **bridge público**; e todo oráculo desta suíte que procure rótulo canônico na tela aplica a mesma transformação. Mapa indisponível = **FAIL nomeado**, nunca comparação crua — um oráculo que não consegue aplicar a transformação declarada está cego, e cego é vermelho |
...
| C5 | **A narrativa continua string pura (INV-7).** Colorir não injeta markup no pipeline determinístico nem altera o texto | `D009-DOM2` · `tests_009_leitura.js` · fixtures `P52_F1`, `P52_F3` e `P52_F5` · `window.__DEV.buildExecutiveNarrative(snap).paragraphs` não contém `<` nem `>`; `trace.length === paragraphs.length`; a `.jn-narrative` tem um `<p>` por parágrafo; e o `textContent` de cada `<p>` é **byte a byte** igual a `__P52.applyCopy(paragraphs[k])` — a transformação de apresentação da 5.2 é **pública, anterior a esta demanda e obrigatória no oráculo** (E2 da errata), e mapa indisponível é FAIL nomeado, nunca comparação crua | **M6**: injetar `<span>` na string de P1 (ramo dos extremos de domínio) → FAIL |
