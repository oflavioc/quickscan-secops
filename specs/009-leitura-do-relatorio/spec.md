...
## Errata de fidelidade — registro da autorização (2026-08-28)

**Trilha de auditoria.** Descoberta na Fase 4, quando o `qa-engineer` traduziu os
critérios em gate: três cláusulas desta spec descrevem algo diferente do que os
gates provados afirmam. **Nenhuma é erro de implementação.** O produto está
provado (`d009` 15 PASS · 0 FAIL, campanha `d009` 18 KILL / 18, `p52layout` 45/0,
`p50core` 64/0) e **nenhuma linha de código muda por causa desta errata**: o que
muda é o texto, que passa a descrever o comportamento já implementado. O
`qa-engineer` não resolveu nenhuma das três em silêncio — implementou a leitura
coerente e escalou, que é o rito.

| Campo | Registro |
|---|---|
| **Quem autorizou** | O proprietário |
| **Quando** | 2026-08-28 |
| **Onde** | No chat, com a frase literal *"Autorizo, peça a errata"* |
| **Alcance** | A redação dos critérios **C5**, **C8** e **C10**. Nenhuma mudança de comportamento, nenhum gate reescrito, nenhuma contagem alterada, nenhum mutante afetado |
| **O que NÃO é reaberto** | As três seções de ratificação anteriores — troca de âncora, ampliação C13 e autorização nominal §29.4 — permanecem **exatamente** como estão. A §8 da 5.2 segue substituída; C13 segue ratificado; a autorização §29.4 segue nominal e restrita à 009 |

### E1 · C10 contradizia C13 — prevalece a ratificação posterior

| Campo | Registro |
|---|---|
| **O que estava escrito** | C10 exigia "**zero** `.ux-tgt-en` com a frase de NONE" sob landscape 100% UNSET |
| **Por que estava errado** | **C13 foi ratificado depois** e manda o estado **S4** manter a frase de S3 (§5, linha S4). Na fixture `P52_F2`, três das quatro práticas-alvo são S4 — `mandate` e `governance` (`soc-governance`) e `team-capacity` (`soc-staffing`), ambas com `landscapeEnabled: false` — e apenas `logs` (`security-analytics`) é S2. Lido ao pé da letra, C10 tornava C13 **impossível de satisfazer**: a mesma tela teria de publicar e não publicar a frase |
| **O que passa a valer** | A cláusula é **por prática**, não global: nenhuma prática **em S2** publica a frase de NONE nem renderiza `.ux-tgt-en`. As práticas em **S4** mantêm a frase, como C13 exige |
| **Onde o gate já afirma isso** | `D009-UNS1` filtra por estado (`porEstado.S2.indexOf(qid) >= 0`) antes de comparar a frase, e exige que a fixture tenha ao menos uma prática em S2 — senão declara o gate vacuoso |
| **Precisão adicional (mesmo critério)** | O texto dizia que a lista do aviso é "**subconjunto** das práticas renderizadas". O gate é mais forte: exige **igualdade** entre a lista nomeada e o conjunto das práticas em S2, e que todas estejam entre as renderizadas no mesmo passe (B5). O texto não era falso — era mais fraco que a prova; passa a descrever a asserção real |
| **Classe** | Inconsistência interna da spec; a ratificação posterior prevalece |

### E2 · C5 omitia o mapa de apresentação público da 5.2

| Campo | Registro |
|---|---|
| **O que estava escrito** | comparação **byte a byte** do `textContent` de cada `<p>` contra `paragraphs[k]`, sem mais nada no caminho |
| **Por que estava errado** | Entre a string canônica e o que a tela mostra existe uma transformação **pública e anterior a esta demanda**: o mapa de apresentação da 5.2 (`ui_p52_workspace_v32.js:128`), publicado como **dado** por `__P52.copyMap()`. Ele troca, entre outros pares, `"Mandato e objetivos"` → `"Direcionamento e objetivos"` e `"mandato"` → `"direcionamento"`. Comparar sem aplicá-lo mediria uma divergência da **5.2**, não o efeito da 009 |
| **Consequência se não fosse corrigido** | O dano não estava só em C5. Os oráculos de `D009-UNS1`, `D009-UNS3` e `D009-UNS4` procuram o **rótulo da prática** dentro do texto do aviso. Sem a transformação, procurariam na tela um rótulo que ela nunca mostra: as asserções **negativas** ("o aviso não nomeia X") passariam **vacuosamente** e as positivas falhariam. A omissão do texto era, portanto, um caminho direto para gate cego |
| **O que passa a valer** | A comparação é byte a byte contra o resultado do mapa público `__P52.copyMap()`, **aplicado pelo próprio oráculo** sobre `paragraphs[k]`; e todo oráculo desta suíte que procure rótulo canônico na tela aplica a mesma transformação. **Por que o oráculo não chama `__P52.applyCopy()`:** usar a função do módulo sob teste tornaria a asserção **equivalente por construção** — ela concordaria com o produto ainda que a substituição estivesse errada. O oráculo lê o mapa como **dado** e reimplementa a substituição (`tests_009_leitura.js:167-176`), e é isso que lhe dá poder discriminante. Alinhar o gate a uma redação que mandasse chamar `applyCopy()` **enfraqueceria** o oráculo — exatamente o dano que esta errata existe para evitar. Mapa indisponível = **FAIL nomeado**, nunca comparação crua: oráculo que não consegue aplicar a transformação declarada está cego, e cego é vermelho |
| **Classe** | A spec omitia transformação que existe, é pública e é anterior à demanda |

### E3 · C8/B13 é cláusula sem caso real — e isso passa a estar escrito

| Campo | Registro |
|---|---|
| **O que estava escrito** | "declarada sem verbete → nenhum `.v32-caphelp`", como se o caso fosse alcançável |
| **Fato verificado** | No runtime congelado há **25** capabilities; **22** têm `landscapeEnabled: true` — são exatamente as chaves de `V32.TECH_LANDSCAPE`, isto é, as declaráveis — e **todas as 22 têm verbete** em `P52_CAP_HELP`. As 3 restantes (`soc-governance`, `soc-staffing`, `soc-skills`) não são declaráveis e não têm verbete. Logo **não existe hoje capability declarável sem verbete**: o cruzamento de B13 é vazio |
| **O que passa a valer** | A cláusula **continua escrita e o gate continua a afirmando** — mas o texto agora declara que o cruzamento é vazio hoje e que o gate guarda a **invariante que o mantém vazio**: se alguém criar uma capability declarável sem verbete, `D009-GLO1` falha nomeando as órfãs, em vez de o ramo seguir passando em silêncio |
...
| C5 | **A narrativa continua string pura (INV-7).** Colorir não injeta markup no pipeline determinístico nem altera o texto | `D009-DOM2` · `tests_009_leitura.js` · fixtures `P52_F1`, `P52_F3` e `P52_F5` · `window.__DEV.buildExecutiveNarrative(snap).paragraphs` não contém `<` nem `>`; `trace.length === paragraphs.length`; a `.jn-narrative` tem um `<p>` por parágrafo; e o `textContent` de cada `<p>` é **byte a byte** igual ao resultado do mapa público `__P52.copyMap()`, **aplicado pelo próprio oráculo** sobre `paragraphs[k]` — a transformação de apresentação da 5.2 é pública, anterior a esta demanda e obrigatória; o oráculo **não** chama `__P52.applyCopy()`, porque usar a função do módulo sob teste o tornaria equivalente por construção (E2 da errata); mapa indisponível é FAIL nomeado, nunca comparação crua | **M6**: injetar `<span>` na string de P1 (ramo dos extremos de domínio) → FAIL |
...
| C8 | **Explicação de uma frase por capability declarada, na tela.** Reuso de `P52_CAP_HELP`, neutro e sem produto; sem verbete, sem texto | `D009-GLO1` · `tests_009_leitura.js` · para cada capId com `V32.TECH_LANDSCAPE[id].presence !== "UNSET"` **e** verbete em `P52_CAP_HELP`, a `.v32-decl-row` correspondente traz um `.v32-caphelp` cujo texto é **a primeira frase** do verbete canônico (transformação declarada e pública — precedente `__P52.copyMap()`); declarada sem verbete → nenhum `.v32-caphelp` — **cláusula sem caso real hoje**: as 22 capabilities declaráveis têm verbete, e o gate guarda a invariante que mantém o cruzamento vazio, falhando se alguém criar uma declarável sem verbete (E3 da errata); capability UNSET → nenhum `.v32-caphelp`; o texto não casa `/Forti[A-Z]/`; a contagem de `.v32-decl-row` (consumida por `p52ContextSummary`) não muda. Lista de declaradas derivada de `V32.TECH_LANDSCAPE`, nunca do DOM | **M9**: `capHelpLine()` devolver o verbete inteiro → FAIL (mais de uma frase). **M10**: emitir a frase para capability UNSET → FAIL |
| C9 | **A mesma explicação no relatório impresso** (P14: glossário acompanha o papel; a ordem do papel, não) | `D009-GLO2` · `tests_009_leitura.js` · em `#pr-landscape`, cada `.pr-card` de capability declarada com verbete traz a MESMA frase de C8; sem verbete → nada; a ordem das seções do relatório impresso permanece a pinada por `tests_p50_core.js:3796-3810` | **M11**: remover a emissão no `buildPrintReport` → FAIL |
| C10 | **UNSET não fala como NONE, e o bloco de ausência não renderiza.** Sob contexto não informado o relatório não conclui nada sobre processo/pessoas/governança | `D009-UNS1` · `tests_009_leitura.js` · fixture `P52_F2` · landscape 100% UNSET com ≥1 prática-alvo em **S2**: nenhuma prática **em S2** renderiza `.ux-tgt-en` nem publica a frase de NONE — as práticas em **S4** mantêm a frase, como C13 exige (E1 da errata); existe **exatamente um** `[data-ux-absence="target-enablers"]`; seu texto declara que o contexto **não foi informado**, não contém "processo, pessoas, governança" e não afirma ausência de tecnologia; a lista que ele nomeia é **exatamente** o conjunto das práticas em S2, e todas estão entre as renderizadas em `.ux-tgt-ovs` no MESMO passe (B5), com os rótulos procurados sob o mapa público `__P52.copyMap()`, aplicado pelo próprio oráculo (E2) | **M12**: devolver a frase única antiga para o ramo UNSET → FAIL (frase de NONE sob UNSET) |
...
|---|---|---|
| `P52-TGT1` (`tests_p52_layout.js:233-268`) | **posição absoluta do alvo** (`iT === iE+1` / `iE+2`) | **Único** que quebra. Reancorado em C3 |
...
(`ui_p52_workspace_v32.css:56-59`), e **apenas ela** — `.p52-sec p`,
...
e é o do próprio relatório impresso: `ui_v32.js:1198-1200` consome
...
- [x] **Specs validadas anteriores — contradição encontrada e RESOLVIDA por troca
  ...
  (`tests_p50_core.js:3796-3810`) permanece byte a byte.
