...
2. **A precedência de C8 tem dois níveis, e sem o segundo C10(c) falha.**
   ...
   `.ux-tgt-enabler` idênticos no mesmo card ⇒ `D010-CARD4` (c) FAIL. A ordem do
...
- **Namespace dos mutantes:** `D010-M1..M17`, na correspondência 1:1 com
  `M1..M17` da spec (R10 §1 — nunca continuar numeração alheia; `M1` global já
  existe desde a 003). Cada par entra em `mutation-matrix.json` com `harness`,
...
| **1 — fixtures** | `fixtures_010_vao.js` com `D010-F1`, `F1b`, `F2`, `F3` + `d010AssertFixtureStates` no padrão da 009. Estado aplicado **só** por owners canônicos (`__DEV.setAnswerById`, `setPriorities`, `setTarget`, editor + `#v32save`) | `qa-engineer` | `chore` | 0 |
...
| **9 — verde e campanha nova** | Fixar a contagem de `d010` no verde; `tests_010_mutants.js` (17 mutantes, **com `--preflight`**) **+** harness `d010` em `mutation_map.json` no mesmo commit; executar `d010`, `d009` e `core`; registrar os 17 pares em `mutation-matrix.json` | `qa-engineer` | `chore` (instrumento de medição; sem red próprio — o aceite é 100% KILL, R10 §5) | 8 |
...
| **Item duplicado no card** (`fortiguard-socaas` do serviço × `SOCaaS` do `MAP`) | `D010-CARD4` (c) | Deduplicação por `data-eid` já é regra do desenho (restrição 2). Sem ela o defeito é silencioso na maioria das sessões e aparece só em `monitoring-coverage`/`incident-response` |
...
3. **A cláusula "classificação ≠ `CONTEXT_NOT_INFORMED`" é load-bearing.** Numa
   sessão com os 15 níveis em 0 e nada declarado, o engine anexa serviços
   (`fortiguard-socaas`, `ir-readiness-subscription`, `ir-plan-development`,
   `ir-playbook-development`, `ttx`, `vulnerability-assessment`, `ir-training`)
   fora do `switch`, por `hasGap` (achado A5). Sem a cláusula, essa sessão
   desligaria a Camada 1 **sem substituir nada**. Com ela: `false`.
4. **A colisão de identidade de `monitoring-coverage` é real** (restrição 2), e é
   o que obriga a precedência de dois níveis.
