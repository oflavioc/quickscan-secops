...
| **9 — verde e campanha nova** | Fixar a contagem de `d010` no verde; `tests_010_mutants.js` (**23** mutantes executados de **26** declarados — `M3`/`M4` em dívida declarada; `M21`..`M25` acrescidos pela errata **E16**, depois de `M20`, sem renumerar id algum —, **com `--preflight`**) **+** harness `d010` em `mutation_map.json` no mesmo commit; executar `d010`, `d009` e `core`; registrar os 17 pares em `mutation-matrix.json` | `qa-engineer` | `chore` (instrumento de medição; sem red próprio — o aceite é 100% KILL, R10 §5) | 8 |
