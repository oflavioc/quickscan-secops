...
[FAIL]  guarda de censo da leitura: merges first-parent até o piso, inclusive: lidos 0 ≠ censo pinado 39 (fecho.json → piso.merges_ate_piso) — leitor mudo ou histórico incompleto; a árvore não pode ser declarada conforme por vácuo      ← só na falha (E016-5)
...
original era *"Exit `1` sse `F > 0`"*). Um FAIL de forma (`branch` ausente, piso inválido,
...
```json
{
  "_meta": {
    ...
    "censo_de_leitura":   "<E016-5: o que é, por quê (J1: leitor mudo deixava a árvore verde por vácuo), medido (dois oráculos = 39), quando se aplica (só com origin/develop presente e piso na cadeia)>",
  ...
  "sonda": { "fixtures": ".claude/verify/fixtures_016/fecho/", "total": 35, "casos": [ { "id": "F1", "esperado": "MESCLADA SEM FECHO" } ] }
}
```
...
**cita o registro como fonte** (R10 §3): era 26 nesta redação, 35 hoje.
...
regra, aditivo e sem renumerar, também nas iterações da Fase 6. A fixture de cada caso declara estados, merges (com
