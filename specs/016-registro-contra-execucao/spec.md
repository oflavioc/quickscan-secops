...
| **C7** | **O julgador não pode ficar mudo.** Toda execução de `check_fecho.py` e de `check_branch_protection.py` roda a sonda sintética antes da árvore real; o total de casos executados é comparado ao **pinado** (`fecho.json → sonda.total`, `branch_protection.json → sonda.total`) e cada veredito ao esperado; divergência ou total diferente ⇒ FAIL nomeando o caso | `D016-FEC1`…`D016-PROT1` (alínea comum, sem gate próprio — é a pré-condição de não-vacuidade dos seis) · `--sonda` emite **JSON em stdout** `{casos:[{id, esperado, obtido, ok}], total, falhas}`; texto humano em stderr. **(E016-5)** A mesma contagem pinada vale para o **leitor** do pós-merge: `piso.merges_ate_piso` × `contagens.merges_ate_piso` (guarda de censo, `check_fecho.py:censo_da_leitura`) — divergência ⇒ `[FAIL]  guarda de censo da leitura: …`, exit 1, sem veredito | `D016-M16` — desligar a sonda (laço vazio) → `total: 0 ≠ pinado`, acusado pelo stage **e** pelo harness, que pina o censo de casos · **(E016-5)** `D016-M33` — leitor mudo (`ler_merges` devolve `[]` com metadados sãos) → `guarda de censo da leitura: FAIL`, exit 1 com **0 problema(s)** de julgamento · **(E016-8)** a guarda aplica-se só com `origin/develop` presente, piso na cadeia **e cadeia íntegra** (`cadeia_integra ≠ false`): leitor que **nomeia** o truncamento ⇒ global `historico-raso` **e** guarda `nao_aplicado` — **um** FAIL por causa; leitor que **cala** com contagem errada ⇒ `divergente`, com a disjunção honesta ("o leitor não nomeou causa: defeito do instrumento ou truncamento que ele não detecta") |
...
(condição necessária, portão barato — `false` ⇒ cadeia íntegra sem mais processo)
e, só se `true`, `git cat-file -p <fim da cadeia>` — ≥ 1 linha `parent` num commit
que a caminhada viu sem pais ⇒ `cadeia_integra: false` (clone raso); sem `parent`
⇒ raiz verdadeira (o flag sozinho acusaria falso — cenário C de
`prova-de-carga.md` §12.1).

**Saída** (stdout, uma linha por sujeito, vocabulário de T10), na ordem:

```
[SONDA] fecho: 35 caso(s) · 0 divergência(s) (total pinado: 35)          ← total no registro (E016-5; era 26)
[INFO]  população: feature/NNN-* ∩ planning-state · piso 921977c2 (merge do PR #39, 2026-09-04) · origin/develop julgado: <sha12>
[INFO]  merges first-parent após o piso: N · até o piso, inclusive: M (não julgados) · censo pinado: 39 (ok)      ← E016-5
[FAIL]  NÃO DETERMINÁVEL (histórico raso: a cadeia first-parent de refs/remotes/origin/develop termina em <fim12>, commit cujo objeto tem pais que o clone não tem; piso 921977c25e76 na posição <N> — git fetch --unshallow origin; git fetch origin develop NÃO repara)      ← só sob clone raso nomeado pelo leitor (E016-8) — EXTRAÍDA de fecho.py:_impedimento e MEDIDA em T096 (prova-de-carga.md §12.2): piso DENTRO do trecho lido — B "termina em 921977c25e76 … na posição 2", G "termina em fdf5779608dc … na posição 2"
[FAIL]  NÃO DETERMINÁVEL (histórico raso: a cadeia first-parent de refs/remotes/origin/develop termina em <fim12>, commit cujo objeto tem pais que o clone não tem; piso 921977c25e76 fora do trecho lido — git fetch --unshallow origin; git fetch origin develop NÃO repara)      ← a outra variante do mesmo global (E016-8), piso FORA do trecho lido — A "termina em ec74d6f79fb5"; só uma das duas aparece (um FAIL por causa), e cada demanda a herda como `NÃO DETERMINÁVEL (fase …) · não julgada — <detalhe>`
[FAIL]  guarda de censo da leitura: merges first-parent até o piso, inclusive: lidos 0 ≠ censo pinado 39 (fecho.json → piso.merges_ate_piso) — o leitor não nomeou causa: defeito do instrumento ou truncamento que ele não detecta; a árvore não pode ser declarada conforme por vácuo      ← só na falha (E016-5; disjunção E016-8, extraída de check_fecho.py:censo_da_leitura — a redação E016-5 dizia "leitor mudo ou histórico incompleto")
...
----
fecho: D demanda(s) · V válvula(s) · F problema(s)[ · guarda de censo da leitura: FAIL]      ← sufixo só na falha (E016-5)
```

Exit `1` sse `F > 0` **ou** a guarda de censo da leitura falha (**E016-5**:
censo `lidos ≠ pinado`, ou `piso.merges_ate_piso` ausente/inválido — a guarda
**não produz veredito** nem conta como problema de julgamento, logo `exit 1 ·
0 problema(s)` é a forma esperada sob leitor mudo, `D016-M33`; a redação
original era *"Exit `1` sse `F > 0`"*). **(E016-8)** Sob histórico raso **nomeado pelo leitor** a forma é `exit 1 · 1 problema(s)` (o global `historico-raso`) com a guarda `nao_aplicado` — um FAIL por causa; `exit 1 · 0 problema(s) · guarda FAIL` fica reservado ao leitor que **cala**. Um FAIL de forma (`branch` ausente, piso inválido, histórico raso,
...
`compliance-audit`) e o local do QA continuam verdes.~~ **(E016-6)** O pipeline
`run.sh` e o local do QA continuam verdes; o check **`verify`**, porém, fica
**vermelho até P2** pelo `compliance-audit` (`D016-PROT1`, T8 c, §Nascimento) —
custo de nascimento, único, com dono, evento e prazo, lido pela razão e não
pela cor; depois de P2 o `verify` volta a significar *o código está são* (a
separação estrutural dos dois sinais é o `EA-36`). Depois de P2, esse vermelho **impede o merge** —
...
| `D016-PR1` | **vermelho no PR da 016 até o `done`** — no check **`fecho`** (E1), por desenho, com nome, e é o **red ao vivo** desta demanda; ~~o check `verify` fica verde~~ **(E016-6: até P2 o `verify` fica vermelho pelo `compliance-audit` — linha `D016-PROT1` abaixo)** | — |
...
| 10 — clone raso / `git` ausente | **(E016-8)** *clone raso* — a ref **está** presente e a cadeia first-parent de `origin/develop` termina em commit cujo objeto tem pais que o clone não tem ⇒ FAIL `NÃO DETERMINÁVEL` com código **`historico-raso`** (C1 g), remédio `git fetch --unshallow origin` (`git fetch origin develop` **não** repara — medido, §12.1 E.1); *`git` ausente / ref ausente* ⇒ FAIL `NÃO DETERMINÁVEL`, nunca SKIP (C1 e), como estava. Redação original: ~~FAIL `NÃO DETERMINÁVEL`, nunca SKIP (C1 e)~~ — **spec-errada**: mandava o raso para C1(e), cujo remédio não conserta |
...
```json
{
  "_meta": {
    ...
    "censo_de_leitura":   "<E016-5: o que é, por quê (J1: leitor mudo deixava a árvore verde por vácuo), medido (dois oráculos = 39), quando se aplica (só com origin/develop presente e piso na cadeia — E016-8: e cadeia íntegra, cadeia_integra ≠ false; leitor que nomeia o truncamento ⇒ nao_aplicado, um FAIL por causa)>",
    "acrescimos_da_fase6": "<E016-5: a regra da Fase 4 reaplicada — aditivo, ids permanentes, nunca renumerar: F24 (C4 e, gap G3), P11 (C5 b, J3)>",
    "acrescimos_ea39":     "<E016-8: a mesma regra — F25 (clone raso, cadeia truncada NO piso), F26 (truncada ACIMA do piso, prova a precedência); contrato_da_sonda.codigos 16 → 17 (historico-raso)>"
  },
  "piso": {
  ...
  "sonda": { "fixtures": ".claude/verify/fixtures_016/fecho/", "total": 37, "casos": [ { "id": "F1", "esperado": "MESCLADA SEM FECHO" } ] }
}
```

**(E016-5)** `piso.merges_ate_piso` é o **censo imutável** de merges
first-parent de `origin/develop` até o piso, inclusive — medido por dois
oráculos (`git rev-list --count --merges --first-parent 921977c…` = 39 =
`fecho.py:ler_merges`), **pinado e nunca calibrado** (R10 §1); lido **só** pela
guarda de censo do gate, nunca pelo julgador. Mutantes de árvore que movem o
piso movem o censo junto (`6dad53d` = 15; raiz `e5ccd429` = 0). `sonda.total`
**cita o registro como fonte** (R10 §3): era 26 nesta redação, 35 na E016-5, **37** hoje **(E016-8)**: F25 e F26.
...
FAIL. Total ~~**26**~~ **(E016-5)** pinado no registro — **35** hoje: a Fase 4
acrescentou **F20–F23** e **P8–P10** (7 casos; descrição e razão em
`fecho.json → _meta.acrescimos_da_fase4`; esperados medidos: F20/F21/F22 →
`NÃO DETERMINÁVEL`, F23 → `MESCLADA SEM FECHO`, P8/P10 → `FECHO PENDENTE`, P9 →
`LIBERADO`) e a Fase 6 acrescentou **F24** (C4 e: prazo `30/09/2026`, fora de
`AAAA-MM-DD` ⇒ `MESCLADA SEM FECHO` · `fecho_pendente-invalida`; carrasco
`D016-M31`) e **P11** (C5 b: `validate` **com** os dois artefatos ⇒ `FECHO
PENDENTE` · `fase-nao-done`; isola a cláusula de fase para `D016-M9`) — a mesma
regra, aditivo e sem renumerar, também nas iterações da Fase 6. **(E016-8)** O fix-finding EA-39 acrescentou **F25** (clone raso com a cadeia truncada **no** piso: `piso_na_cadeia true · cadeia_integra false · fim_da_cadeia = piso · posicao_do_piso 0`, `merges []`, ancestralidade `false` ⇒ `NÃO DETERMINÁVEL` · oráculo `null` · `historico-raso` · 1 problema; carrasco `D016-M34`) e **F26** (truncada **acima** do piso: `piso_na_cadeia false · cadeia_integra false · fim_da_cadeia = ponta · posicao_do_piso null` ⇒ idem — e **não** `piso-invalido`: é a fixture que prova a precedência; carrasco `D016-M35`), total **37**, razões em `_meta.acrescimos_ea39`; `_meta.contrato_da_sonda.codigos` 16 → **17**. Contrato de `origin_develop` (lido pelo julgador e pela guarda de censo): `{presente, sha, causa, piso_na_cadeia}` **+** `cadeia_integra` (`true · false · null` — `false` é o **leitor afirmando** que a cadeia first-parent termina em commit cujo objeto tem pais que o clone não tem; `null` = cadeia não lida), `fim_da_cadeia` (sha40 · `null`), `posicao_do_piso` (int · `null`; `piso_na_cadeia` permanece e vale `posicao_do_piso is not None`) — os consumidores leem **`is False`**: ausente/`null` = não afirmado = comportamento anterior; F1–F24 não trazem os campos e permanecem intactas (a forma estrita `is not True` rotularia um leitor que esqueceu o campo como "clone raso", o que a decisão de forma proíbe). A fixture de cada caso declara estados, merges (com
...
**(E016-5)** Hoje: **33 pares** `D016-M1..M33` (30 na wave 5; M31–M33 na
Fase 6), **54 targets enumerados um a um** (`check_mutation.py` casa por
pertinência exata — `fixtures_016/**` não casa nada; obtidos por `git
ls-files`; incluem os dois planning-states que a campanha muta e o inexistente
`999-sintetica-d016.json`, para que um commit que o crie redispare a campanha),
3 controles (`C0-fecho`, `C0-protecao`, `D016-M24/positivo`). Famílias além
das de instrumento/gate/árvore: **`criar`** (M32 — âncora = **ausência** do
arquivo, restauração = unlink conferido) e **leitor** (M33 — a sonda não vê
leitor; quem mata é a guarda de censo do gate nu, e o kill exige **0
problema(s)** de julgamento). Mutantes de árvore pinam sujeito, total de
problemas **e isolamento** (nenhum outro sujeito com falha).
**(E016-8)** Com o fix-finding EA-39: **35 pares** (`D016-M34`/`D016-M35`, instrumento,
mortos pela sonda em F25/F26 — entram com o green de `fecho.py`, nunca antes: ancorar
em código inexistente seria âncora podre no preflight), **56 targets** (+`F25.json`,
+`F26.json`, no commit red). **Sem mutante de árvore para a metade do leitor** — razão
medida, não preguiça: o harness restaura por bytes + SHA-256 + `git status --porcelain`
escopado e `.git/shallow` é invisível aos três; `git fetch --depth=1` numa worktree muta
o `.git` **compartilhado pelas nove worktrees** desta máquina; `--unshallow` exige o
remoto e falha em repositório completo. Carrasco nomeado: **bateria adversarial de I/O
em clone efêmero com `origin` = GitHub** (`prova-de-carga.md` §12), reexecutada a cada
mudança de `ler_merges`; dívida em `mutation-matrix.json → dividas_declaradas`.
`D016-M33` não muda: a âncora `return {"merges": merges, "origin_develop": od}` fica
byte-idêntica e única, e os campos novos são populados **antes** dela.
...
| `.github/workflows/verify.yml` | job próprio **`fecho`** (T8 b, E1); ~~jobs `verify`/`visual` e~~ **`:42` intocado**; **(E016-6)** o passo do `compliance-audit` no job `verify` ganhou `env: GITHUB_TOKEN: ${{ github.token }}` (gap G6, `9a460f5`) — sem isso o gate lia a API **anonimamente** e a medição de §Não mensurável 1 nunca acontecia |
...
| **(E016-8)** `.claude/verify/fecho.py` · `check_fecho.py` · `fecho.json` · `fixtures_016/fecho/F25.json` · `F26.json` · `tests_016_mutants.js` · `mutation_map.json` · `mutation-matrix.json` · `specs/016-…/spec.md` · `plan.md` · `prova-de-carga.md` · `.claude/BACKLOG.md` | fix-finding **EA-39** (2026-09-05): código `historico-raso`; `gen_pins.py` em commit separado após cada commit de conteúdo (R8 §1) |

**Intocados, por decisão**: `check_state.py`, `check_tdd.py`,
~~`check_mutation.py`~~ (**E016-7**: ecoa os blocos `CONTROLE` do harness —
relato, não veredito; continua fora de todo `targets`), `env_doctor.py`, `expected_suites.json` (nenhuma suíte nova
...
## Erratas da Fase 6 (2026-09-04) — numeração `E016-N`

**Por que o prefixo.** A próxima errata desta spec seria "E5" — o mesmo id do
achado histórico `E5` (`pins.md:8`, o MANIFEST sempre vermelho) que a errata E4
acabou de desambiguar de `EA-5`, depois de sete citações trocadas. Numerar
"E5" reabriria a confusão no arquivo que a corrigiu. A partir daqui as erratas
desta spec levam o prefixo da demanda — **`E016-5`, `E016-6`, `E016-7`** —, na
forma dos ids `D016-*`; **E1–E4 mantêm os ids com que já foram citadas**
(renumerar quebraria citações — R12 §Achados). Células emendadas levam a marca
`(E016-N)`; a redação original de cada uma está preservada abaixo (R2 §5).

### E016-5 · G4 — a guarda de censo da leitura: "exit 1 sse F > 0" ficou atrás do gate; itens 36/56/59 (errata aditiva)

| Campo | Registro |
|---|---|
| **Proveniência** | Autorização para escrever esta errata: **LITERAL do usuário no chat, 2026-09-04** — *"Escrever as duas erratas e seguir"*, depois da escalada da iteração 2 do `spec-validate` (67/70, abaixo de 100%, como a skill manda). O **conteúdo** que ela registra — a guarda de censo, `D016-M33`, F24/P11 — foi **DECIDIDO SOB DELEGAÇÃO DO PROPRIETÁRIO de 2026-08-29, não aprovado por ele pessoalmente**: candidata J1 da iteração 1, implementada em `9fe57f0`, ratificada pelo `product-owner` no aceite condicionado (*"além do que J1 pedia"*). Escrita pelo `qa-engineer` |
| **O que estava escrito** | §Superfície 1: *"Exit `1` sse `F > 0`"*; amostra com `[SONDA] fecho: 26 caso(s) · 0 divergência(s) (total pinado: 26)`, `[INFO]  merges first-parent após o piso: N · anteriores ao piso: M (não julgados)` e fecho `fecho: D demanda(s) · V válvula(s) · F problema(s)`. §Contratos `fecho.json`: `piso {sha, descricao}`, `sonda.total: 26`, *"Total **26**"*. T3 e C7: a contagem pinada descrita só para a **sonda**. §Contratos harness: famílias de instrumento (*"`reason` sobre os ids de caso da sonda"*), sem `criar` nem leitor |
| **Fato medido** (2026-09-04, iteração 2 do `spec-validate`, item 53; clone efêmero) | Sob `D016-M33` (`ler_merges` devolve `[]` com metadados sãos) o gate de HEAD sai **`exit 1 · 0 problema(s)`**: `fecho: 11 demanda(s) · 0 válvula(s) · 0 problema(s) · guarda de censo da leitura: FAIL` — "sse F > 0" é falso **por desenho ratificado**. Sem a guarda (gate de `76fd9dc`) o mesmo leitor mudo dava `exit 0 · 0 problema(s)`: a árvore ficava verde **por vácuo** e `D016-M19` sobrevivia (J1). A guarda é correta; a spec é que ficou atrás. Junto vieram, sem errata própria (itens 36/56/59): sonda 33 → **35** (F24, P11), `piso.merges_ate_piso = 39`, targets 51 → **54**, pares 30 → **33** |
| **O que passa a valer** | (a) §Superfície 1: exit `1` sse `F > 0` **ou** a guarda de censo da leitura falha (censo `lidos ≠ pinado`, ou pin ausente/inválido); a guarda **não produz veredito** e não conta como problema de julgamento — `exit 1 · 0 problema(s)` é a forma esperada sob leitor mudo; linhas `[INFO]  … · até o piso, inclusive: M (não julgados) · censo pinado: 39 (ok)` e, só na falha, `[FAIL]  guarda de censo da leitura: <detalhe>`; fecho `fecho: … · F problema(s)[ · guarda de censo da leitura: FAIL]` — formato **extraído de `check_fecho.py:relata_pos`**, não redigido. (b) §Contratos `fecho.json`: `piso.merges_ate_piso` — censo **imutável** de merges first-parent até o piso, inclusive; dois oráculos (`git rev-list --count --merges --first-parent 921977c…` = 39 = `fecho.py:ler_merges`); **pinado e nunca calibrado** (R10 §1); `_meta.censo_de_leitura` (o que é, por quê, medido, quando se aplica — só com `origin/develop` presente e piso na cadeia; fora disso o global do julgador nomeia a causa); `sonda.total` **cita o registro como fonte** (R10 §3) e a regra "aditivo, ids permanentes, nunca renumerar" vale também para iterações da Fase 6 (`_meta.acrescimos_da_fase6`: F24, P11). Mutantes de árvore que movem o piso movem o censo junto (`6dad53d` = 15; raiz `e5ccd429` = 0). (c) T3/C7: a contagem pinada aplica-se também ao **leitor**, via censo, com o **limite declarado** — merges **posteriores** ao piso não têm censo pinável; leitor que os perde é indistinguível de "0 merges após o piso" por asserção do stage; carrasco só na campanha (dívida em `mutation-matrix.json → dividas_declaradas`). (d) §Contratos harness `d016`: 33 pares, 54 targets um a um, famílias **`criar`** (M32) e **leitor** (M33) |
| **Onde o gate já afirma isso** | `check_fecho.py:censo_da_leitura` + `relata_pos` (`9fe57f0`); `fecho.json → piso.merges_ate_piso`, `_meta.censo_de_leitura`, `_meta.acrescimos_da_fase6`; `D016-M33` (KILL exige `censo.estado = divergente`, `pinado 39`, `lido 0` **e** `0 problema(s)` de julgamento); F24 × `D016-M31`; P11 × `D016-M9`; o controle `C0-fecho` exige `censo ok` no baseline. Campanha `d016` 33/33 (`prova-de-carga.md` §9.4) |
| **O que NÃO muda** | A guarda **não sai** para devolver o "sse" — seria R10 §1 ao contrário (nunca enfraquecer o gate para casar a letra). Nenhum id renumera; nenhum critério cai; nenhuma boundary se amplia |
| **Classe** | **spec-errada** (skill `spec-validate` §3), errata **aditiva**: a implementação, ratificada, é mais forte que a letra; a letra sobe até ela |

### E016-6 · G5 — "o `verify` fica verde" é falso até P2, e a própria spec o torna falso (doc-only)

| Campo | Registro |
|---|---|
| **Proveniência** | Autorização para escrever esta errata: **LITERAL do usuário no chat, 2026-09-04** — *"Escrever as duas erratas e seguir"*. A decisão de **não** mover `D016-PROT1` para fora do job `verify` nesta demanda e registrar a conflação como achado (**`EA-36`**, `b4b458a`) foi do orquestrador — **DECIDIDA SOB DELEGAÇÃO DO PROPRIETÁRIO de 2026-08-29, não aprovada por ele pessoalmente**. Escrita pelo `qa-engineer` |
| **O que estava escrito** | T8 (custo aceito): *"o check `fecho` do PR fica vermelho por desenho e com nome (borda 6); o `verify` fica verde"*; E1 (iv): *"Borda 6: o vermelho até o `done` vive no check `fecho`; o `verify` fica verde"*; §Superfície 2: *"O check `verify` (pipeline `run.sh` + `compliance-audit`) e o local do QA continuam verdes"*; §Nascimento, linha `D016-PR1`: *"o check `verify` fica verde"*; tabela de arquivos: *"jobs `verify`/`visual` e `:42` intocados"* |
| **Fato medido** | Run `33927191969` (2026-09-04), job `verify` = **FAILURE**: `verify: 16 PASS · 0 FAIL` no pipeline e, no passo seguinte, `[FAIL] branch-protection: develop DESPROTEGIDA · faltam: fecho, up-to-date, verify, visual · …` · `compliance: 15 PASS · 1 FAIL · 0 WARN` · exit 1. A própria spec põe `D016-PROT1` no `compliance-audit` **do job `verify`** (T8 c, `verify.yml:43-44`) e o declara **vermelho até P2** (§Nascimento): no intervalo pré-P2 as duas frases eram conjuntamente insatisfazíveis. No mesmo run o passo do audit lia a API **anonimamente** (G6) — corrigido em `9a460f5` com `env: GITHUB_TOKEN: ${{ github.token }}` no passo, o que também torna falso o "job `verify` intocado" da tabela de arquivos |
| **O que passa a valer** | Nas quatro frases: *até P2, o check `verify` fica **vermelho** pelo `compliance-audit` (`D016-PROT1`, §Nascimento) — custo de nascimento, único, com dono (proprietário), evento (P2) e prazo (antes do merge deste PR), lido pela razão e não pela cor; depois de P2, `verify` volta a significar "o código está são"*. O pipeline `run.sh` e o local do QA continuam verdes (isso **era** e **é** verdadeiro). A separação estrutural dos dois sinais fica em **`EA-36`**: a forma barata é mover a seção `branch-protection` para o job `fecho` — desenho de CI, fora desta demanda (recomendação ao `tech-lead`). Tabela de arquivos: `verify.yml` — job `fecho` (E1) **e** `env: GITHUB_TOKEN` no passo do audit do job `verify` (G6); `:42` (`MUTATION_DEFER_MISSING`) segue intocado |
| **O que NÃO muda** | Nenhum código, nenhum gate, nenhum escopo: `D016-PROT1` continua no `compliance-audit` do job `verify`; o vermelho continua sendo **exatamente o que o gate existe para acusar** (§Nascimento). A frase é que estava errada — não a decisão |
| **Classe** | **spec-errada**, doc-only: exigência formulada sem considerar T8 (c) × §Nascimento; falsificada por execução até P2 |

### E016-7 · T9 — `check_mutation.py` deixa de ser intocado: ecoa os blocos `CONTROLE … resultado:` do harness (relato, não veredito)

| Campo | Registro |
|---|---|
| **Proveniência** | **DECIDIDO SOB DELEGAÇÃO DO PROPRIETÁRIO de 2026-08-29, não aprovado por ele pessoalmente.** Decisão do orquestrador em 2026-09-04, que relata "ok" explícito para uma mudança de **relato** (não de veredito) do instrumento de campanha; este agente não pode verificar o chat — registrado como delegada até confirmação. Executor e dono do arquivo: `qa-engineer` (013 T002/T004; harness `d016`, T060) |
| **O que estava escrito** | T9: *"Nada muda em `check_state.py`, `check_tdd.py`, `check_mutation.py`, `verify.yml:42` … nem na spec 013"*; tabela de arquivos, *"Intocados, por decisão: … `check_mutation.py` …"*; `spec-validate.md` §A1, direção 1: *"O bloco de não-KILL do `check_mutation.py` — intocado, T9 — já repete a nota"* |
| **Fato medido** | Run `33927191969`, job `visual`: 13 mutantes de árvore `NÃO EXECUTADO · baseline do gate nu VERMELHO`, **causa não isolável pelo log** — o harness imprimiu `CONTROLE C0-fecho · resultado: FALHOU · <nota>` e o stage a descartou por construção (`mut_ler` casa só os três estados; o `tail` de 2 linhas devolve as duas últimas entradas da lista de fecho). Reproduzido em clone efêmero (2026-09-04, `origin/develop` = `921977c`) em **quatro formas de C0 vermelho** — fixture extra (`F99.json`), `origin/develop` ausente, `check_fecho.py` quebrado, piso fora da cadeia (`origin/develop` = `6dad53d`): em HEAD nenhuma linha `CONTROLE` chega ao stage e a nota do mutante é a constante; sob fixture extra o próprio controle imprimia `undefined demanda(s) · undefined problema(s) · censo da leitura undefined/undefined (undefined)` (`vivo: null`), e **`D016-M1` saía DETECTADO com o C0 vermelho** — `julgaSonda` não pina isolamento, logo um C0 vermelho não localiza a falha numa metade. É a família do `EA-15` (veredito sem motivo), dentro do instrumento de campanha |
| **O que passa a valer** | (i) `check_mutation.py` ganha `mut_controles()`/`mut_relata_controles()`: cada bloco `CONTROLE  <id> · <desc>` / `resultado: OK\|FALHOU[ · nota]` sai como `controle: <id> · <resultado> · <nota>` logo após o tail e antes dos não-KILL; contagem lida × `controles` do preflight (C1) ⇒ `LEITURA PARCIAL` nomeada; teto de 15 com sobra nomeada; harness sem controles ⇒ nada ecoado; **nada move `fails`** — o exit do harness já carrega a falha do controle. (ii) Harness `d016`: a nota de todo `NÃO EXECUTADO` por baseline (e do controle positivo "não medido") carrega `controle <id> · resultado: FALHOU · <nota>`; `julgaControle` nomeia a **metade que falhou** (SONDA / ÁRVORE), cobre `vivo: null`, JSON ausente (exit + última linha do stderr), erro de leitura, globais (`origin-develop-ausente`, `piso-invalido`) e acusados, e nunca imprime `undefined`. (iii) T9 continua verdadeiro para `check_state.py`, `check_tdd.py`, `verify.yml:42` e a spec 013; a cláusula C4 da 013 segue literalmente verdadeira; `check_mutation.py` continua fora de todo `targets` (nenhuma campanha existente dispara por ele — a `d016` redispara pelo próprio harness, que é target de si mesmo) |
| **Onde o gate já afirma isso** | Bateria unitária do leitor — funções extraídas por AST de HEAD e do arquivo emendado, alimentadas com as saídas reais dos cinco cenários mais nove sintéticos (**29/29**, `mut_ler` byte-idêntico nos dois ⇒ veredito intocado); stage `mutation` inteiro no clone com a fixture extra commitada: `[RUN] d016` → 3 linhas `controle:` + `não-KILL: 13 de 33 · 20 KILL` + `mutation: 1 problema(s)` — a forma exata do log do job `visual`, agora com a razão. Registro: `prova-de-carga.md` §10 |
| **Classe** | Mudança de **relato** do instrumento; nenhuma asserção enfraquece; nenhum veredito, id, contagem ou boundary muda. Fortalece: o `NÃO EXECUTADO` por baseline passa a ser atribuível |

## Errata pós-fecho — fix-finding EA-39 (2026-09-05) — numeração `E016-N`

A demanda 016 está `done` e mesclada (PR #40, `1f7d039`); esta errata nasce de um
`fix-finding` (skill própria, sem spec nova) sobre o achado **`EA-39`**
(`.claude/BACKLOG.md`), na branch `fix/ea39-leitor-mudo`. Mesma regra das erratas
da Fase 6: célula emendada leva a marca `(E016-8)`, a redação original fica
preservada (R2 §5), nenhum id renumera.

### E016-8 · EA-39 — o leitor nomeia o histórico raso: 17º código `historico-raso`, veredito `NÃO DETERMINÁVEL`, precedência forma → ausente → **raso** → piso fora da cadeia (errata aditiva; borda 10 spec-errada)

| Campo | Registro |
|---|---|
| **Proveniência** | **DECIDIDO SOB DELEGAÇÃO DO PROPRIETÁRIO de 2026-08-29, não aprovado por ele pessoalmente.** A delegação é **geral** (*"tome as decisões por mim"*, `.claude/agent-memory/doc-writer/project_delegacao-proprietario-2026-08-29.md`) e **não enumera "errata"** — a subsunção é do orquestrador (2026-09-05) e está escrita aqui para poder ser contestada. O `product-owner` registrou que a errata segue **sem ratificação do proprietário no chat** e leva esta fórmula (BACKLOG §EA-39, "Errata da spec 016"); a ratificação nominal do usuário seria a autorização mais forte e **não foi pedida** nesta rodada — delegação nunca se promove a autorização mais forte. Escrita pelo `qa-engineer` (T090), com o texto técnico do `tech-lead` (`ea39-desenho.md` §7) |
| **Quem decidiu** | **Forma** — código novo `historico-raso`, veredito `NÃO DETERMINÁVEL`, T10 intacto, precedência, guarda de censo mantida, regra de composição "um FAIL por causa": `product-owner`, `.claude/BACKLOG.md` §EA-39 "Decisão de forma" (`e2d3892`). **Detecção, campos, lugar, tipagem `fix`**: `tech-lead`, `specs/016-registro-contra-execucao/ea39-desenho.md` (`fac8bfd`). Ambos sob a delegação; nenhum ratificado pelo proprietário |
| **Quando** | 2026-09-05 |
| **Onde** | Fases 2+3 do `fix-finding` (desenho) → T090 (esta errata, **antes do red** T091 — o `spec-validate` extrai critérios da spec, e F25/F26/M34/M35 precisam existir nela antes do primeiro red) |
| **O que estava escrito** | §Casos de borda, linha 10: *"clone raso / `git` ausente → FAIL `NÃO DETERMINÁVEL`, nunca SKIP (C1 e)"*. C1: alíneas (a)–(f), sem estado para cadeia truncada; C2(d): `piso-invalido` sem ressalva de cadeia íntegra; C7 / E016-5 (b)(c): guarda "só com `origin/develop` presente e piso na cadeia"; §Superfície 1: Entrada sem detecção de raso; Saída com `[FAIL]  guarda de censo da leitura: … — leitor mudo ou histórico incompleto`; §Contratos: `origin_develop {presente, sha, causa, piso_na_cadeia}`, `sonda.total` 35, `codigos` 16, 33 pares, 54 targets; `fecho.py`, decisão 2 do cabeçalho: precedência com **dois** elos (forma → ausente → piso fora da cadeia) |
| **Fato medido** (T092, 2026-09-05 — clones efêmeros no scratchpad com `origin` = **GitHub**, `git 2.55.0.windows.4`, python 3.14.7; código de HEAD `31eb1a4`: `fecho.py ba4ce6cc…`, `check_fecho.py 2ab6908e…`, `fecho.json 2e043cac…` são os mesmos blobs de `origin/develop` `ec74d6f`; tabela integral em `prova-de-carga.md` §12.1) | **A** `git fetch --depth=1 origin develop` (raso **acima** do piso — `develop` já tem 2 merges depois de `921977c`): `.git/shallow` = `ec74d6f`, cadeia `[ec74d6f]` com `%P` vazio e objeto com **2 `parent`**; leitor `{presente true, piso_na_cadeia false, causa null}` ⇒ global **`piso-invalido`** com o detalhe **falso** *"um SHA de outra branch não é piso"*, 11/11 `NÃO DETERMINÁVEL`, censo `nao_aplicado`, `1 problema(s)`, exit 1. **B** `--depth=3` (cadeia termina **no** piso — assinatura do EA-38): `.git/shallow` com 3 linhas, `921977c` com `%P` vazio e objeto com 2 `parent`, piso na posição 2; leitor `{presente true, piso_na_cadeia true, causa null}` ⇒ **leitor mudo**: `globais []`, **`0 problema(s)`**, `em_voo 10` (as dez `done` mescladas saem `[OK] … EM VOO`), 016 `CONFORME`, `merges_ate_piso 0`; só a guarda acusa (`divergente 0 ≠ 39`), exit 1. **C** clone completo + `git fetch --depth=1 origin c4ecfec` (o comando do Playwright, commit **fora** da cadeia first-parent): `is-shallow-repository = true`, cadeia íntegra (69 commits até a raiz `e5ccd42`, 0 `parent`) ⇒ `39 (ok) · 0 problema(s) · exit 0`, `--json` byte-idêntico ao baseline do mesmo clone — o flag sozinho acusaria **falso**. **D** `--pr --json` em A (raso) e em C (completo), `pull_request` sobre `develop`: byte-idênticos (016 ⇒ `LIBERADO`; `fix/ea39-leitor-mudo` ⇒ `NÃO JULGADO · fora-da-populacao`) — o caminho `--pr` não lê histórico. **E** em B: `git fetch origin +refs/heads/develop:refs/remotes/origin/develop` (sem `--depth`) **persiste** raso e o gate repete B; `git fetch --unshallow origin` (contra o GitHub, 9 s) restaura — 69 commits, `39 (ok) · 0 problema(s) · exit 0`. A ressalva do protótipo do `tech-lead` (§0 do desenho: o `--unshallow` puxou o `develop` **local**, atrás do remoto) está fechada: aqui todo `origin` é o GitHub |
| **O que passa a valer** | (1) §Casos de borda, linha 10 — separada: clone raso ⇒ `NÃO DETERMINÁVEL` com código **`historico-raso`** (C1 g) e remédio `git fetch --unshallow origin`; `git`/ref ausente ⇒ C1(e) como estava. **Classe spec-errada**: a borda mandava o raso para C1(e), cuja ref **está** presente (A/B) e cujo remédio (`git fetch origin develop`) **não repara** (E). (2) **C1(g)**: cadeia first-parent de `origin/develop` truncada por clone raso — o leitor afirma `cadeia_integra: false` — ⇒ FAIL `NÃO DETERMINÁVEL`, código `historico-raso`, detalhe com o SHA em que a cadeia termina (`fim_da_cadeia`), a posição do piso (`posicao_do_piso`, ou "fora do trecho lido") e o remédio; **C2(d)**: `piso-invalido` por "fora da cadeia" só com cadeia íntegra — precedência dos globais **forma → `origin/develop` ausente → raso → piso fora da cadeia** (a decisão 2 do cabeçalho de `fecho.py` ganha o terceiro elo). (3) **C7 / E016-5 (b)(c)** e `fecho.json → _meta.censo_de_leitura.quando_se_aplica`: a guarda aplica-se só com `origin/develop` presente, piso na cadeia **e cadeia íntegra** (`cadeia_integra ≠ false`). **Regra de composição**: leitor nomeia o truncamento ⇒ global `historico-raso` **e** guarda `nao_aplicado` (**um** FAIL por causa — a regra vigente "fora disso o global do julgador já nomeia a causa", estendida ao campo novo); leitor cala com contagem errada ⇒ guarda `divergente` com a disjunção honesta *"o leitor não nomeou causa: defeito do instrumento ou truncamento que ele não detecta"*. A guarda **fica**: é o oráculo independente que pegou o EA-39 (a lição do EA-5 — autorrelato de instrumento não é evidência). (4) §Superfície 1 — Entrada: `git rev-parse --is-shallow-repository` (condição necessária, portão barato: `false` ⇒ íntegra sem mais processo) e, só se `true`, `git cat-file -p <fim da cadeia>` (≥ 1 linha `parent` num commit que a caminhada viu sem pais ⇒ truncada; sem `parent` ⇒ raiz verdadeira, caso C). Saída: a linha `[FAIL]` do global `historico-raso` será **extraída de `fecho.py:_impedimento` depois do green** (como a E016-5 fez com `relata_pos`), nunca redigida antes — pendência nomeada, fechada no `spec-validate` do fix; **EXTRAÍDA em T096 (2026-09-05), medida em clone efêmero (`prova-de-carga.md` §12.2), nas duas variantes do detalhe — piso dentro do trecho lido:** `NÃO DETERMINÁVEL (histórico raso: a cadeia first-parent de refs/remotes/origin/develop termina em 921977c25e76, commit cujo objeto tem pais que o clone não tem; piso 921977c25e76 na posição 2 — git fetch --unshallow origin; git fetch origin develop NÃO repara)` (B; em G, raso abaixo do piso, `termina em fdf5779608dc … na posição 2`); **piso fora do trecho lido:** `NÃO DETERMINÁVEL (histórico raso: a cadeia first-parent de refs/remotes/origin/develop termina em ec74d6f79fb5, commit cujo objeto tem pais que o clone não tem; piso 921977c25e76 fora do trecho lido — git fetch --unshallow origin; git fetch origin develop NÃO repara)` (A). `relata_pos` a imprime como `[FAIL]  <detalhe>` e cada demanda a herda (`NÃO DETERMINÁVEL (fase …) · não julgada — <detalhe>`); §Superfície 1 — Saída traz as duas formas. A pendência fecha aqui, antes do `spec-validate`; a linha da guarda traz a disjunção nova (extraída de `check_fecho.py:censo_da_leitura`, commit red). Sob raso nomeado a forma é `exit 1 · 1 problema(s)`; `exit 1 · 0 problema(s) · guarda FAIL` fica reservado ao leitor que cala. (5) §Contratos: `origin_develop` ganha `cadeia_integra` (`true · false · null`; `null` = cadeia não lida, nos três `return` antecipados), `fim_da_cadeia` (sha40 · `null`), `posicao_do_piso` (int · `null`; `piso_na_cadeia` permanece e vale `posicao_do_piso is not None`) — **consumidores leem `is False`**: ausente/`null` = não afirmado = comportamento anterior, F1–F24 intactas. Sonda: **F25** e **F26** (ids permanentes, `_meta.acrescimos_ea39`), `total` **37**, `codigos` **17**. Harness `d016`: pares 33 → **35** (`D016-M34`, `D016-M35`, com o green), targets 54 → **56** (no red), **dívida declarada** "leitor sob clone raso" com carrasco nomeado (bateria §12). (6) Tabela de arquivos pinados: linha `(E016-8)`; `gen_pins.py` em commit separado após cada commit de conteúdo (R8 §1). (7) `plan.md` ET3: impeditivos `piso-invalido | origin-develop-ausente | historico-raso`. `tasks.md` da 016 **não muda** (T090–T099 vivem em `ea39-desenho.md` §8) |
| **Onde o gate já afirma isso** | **Red** (T091, medido na árvore de trabalho antes deste commit e do seguinte, `test(EA-39): red — historico-raso`): `check_fecho.py --sonda` ⇒ `[SONDA] fecho: 37 caso(s) · 2 divergência(s) (total pinado: 37)`, guarda `CODIGOS do instrumento ≠ fecho.json → _meta.contrato_da_sonda.codigos — a mais: [] · a menos: ['historico-raso']`, `✗ F25 (pos): veredito: esperado 'NÃO DETERMINÁVEL' · obtido 'EM VOO' · codigo: esperado 'historico-raso' · obtido None · problemas: esperado 1 · obtido 0`, `✗ F26 (pos): codigo: esperado 'historico-raso' · obtido 'piso-invalido'`, exit 1; gate nu ⇒ *árvore não julgada: o julgador reprovou na própria sonda (C7)*, exit 1; `node tests_016_mutants.js --preflight` ⇒ 33 mutantes, `todas as âncoras com ocorrencias == 1` (a âncora de `D016-M33` intacta). `check_fecho.py:censo_da_leitura` já lê `cadeia_integra is False` (inerte até o green: o campo não existe). O green (T094, `fecho.py`), os pares `D016-M34`/`M35`, a campanha e a bateria pós-fix (§12.2) são registrados no fecho do fix, não aqui |
| **O que NÃO é reaberto** | T10 (vocabulário de vereditos); C1(e) para `git`/ref ausente (segue `origin-develop-ausente`, F22); F1–F24 e seus esperados; o piso `921977c` e o censo 39; `D016-M33` (âncora `return {"merges": merges, "origin_develop": od}` byte-idêntica e única — sob M33 o `od` sai íntegro e a guarda continua sendo quem mata); os dois primeiros elos da precedência; a guarda de censo (fica); o caminho `--pr` (não lê histórico — medido, D); `MUTATION_DEFER_MISSING`/T9; EA-38 (vetor, tratado à parte — independente: consertar o vetor não ensina o leitor a falar); truncamento fora da cadeia first-parent e graft/`refs/replace` **não ganham código** (R12: caso hipotético; C mostra que o flag não o distingue) — ficam na dívida de T096 |
| **Classe** | Conjunto **aditivo**; item (1) **spec-errada**. Nada enfraquece: B passa de `exit 1 · 0 problema(s)` (só a guarda) para `exit 1 · 1 problema` nomeado; A troca um detalhe falso por um verdadeiro; C continua `exit 0`. Nenhum id renumera; nenhuma boundary se amplia; nenhum veredito de gate **alheio** muda |
