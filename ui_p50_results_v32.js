...
(function () {
  ...
  /* EA-23 · nome da capability na vista do assessment, quando ele existe e e
     UNICO. Le `MAP` direto — o mesmo owner congelado que este modulo ja consome
     em `p50CapCards()` —, nunca uma copia local. Devolve null quando a
     capability agrega mais de uma pergunta, quando nao tem pergunta alguma, ou
     quando os dois vocabularios ja coincidem. */
  function p50CapAssessedName(cap) {
    var qids = cap && cap.questionIds;
    if (!qids || qids.length !== 1) return null;
    if (typeof MAP === "undefined" || !MAP[qids[0]]) return null;
    var nome = MAP[qids[0]].cap;
    return (nome && nome !== cap.name) ? nome : null;
  }
  function p50PresenceStrip() {
    ...
    for (var i = 0; i < ids.length; i++) {
      ...
      var avaliado = p50CapAssessedName(cap);          /* EA-23 · ver nota abaixo */
      chip.setAttribute("aria-label", cap.name +
        (avaliado ? ", avaliada como “" + avaliado + "”" : "") + ": " +
        (P50_PRESENCE_LABEL[pres] || pres) + " · " + (P50_PRESENCE_ACC[pres] || ""));
      ...
      /* EA-23 · o catalogo e o assessment nomeiam a mesma capability de formas
         diferentes, e este chip e a UNICA superficie do relatorio onde o nome de
         catalogo alcanca o leitor: em toda a outra parte ele le `MAP[qid].cap`
         ("Capability a desenvolver:"). Sem esta linha, quem sai de um gap e vem
         procurar o contexto tecnologico dele nao reconhece a mesma capability.
         SO quando a capability e 1:1 com uma pergunta avaliada: `soc-governance`
         agrega tres (mandate, governance, policies) e nao tem "o" nome avaliado —
         escolher um seria fabricar vocabulario. E so quando os nomes DIVERGEM:
         repetir o mesmo texto duas vezes e ruido, nao sinonimo. */
      if (avaliado) chip.appendChild(el("span",
        { "class": "p50-presence-alias", "data-p50": "presence-alias" },
        "avaliada como “" + avaliado + "”"));
