...
function renderBlocks(app){
  ...
  if (legacy){
    ...
    /* [010 · V1] ARBITRAGEM DE CAMADA — calculada UMA vez por render, aqui, e
       repassada como argumento a quem decide (a ocultação da Camada 1) e a quem
       fala sobre ela (a frase do card base). Antes desta demanda o argumento de
       `hideLegacyRecommendation` era a constante `true`: declarar QUALQUER
       contexto ocultava a recomendação congelada, mesmo quando a camada V3.2 não
       tinha nada para pôr no lugar — o "vão de contexto parcial" (spec §1). */
    const haSubstituto = hasSubstituteV32(lastCtx);
    ...
    hideLegacyRecommendation(app, haSubstituto);   /* [010 · V1] "há substituto?" no lugar da constante `true` */
...
/* [010 · INV-7] O QUARTO PARÂMETRO `afirmaPreservacao` — e por que o default é falsy
   ---------------------------------------------------------------------------------
   Até esta demanda o card base afirmava "Leitura V3.1.3 preservada" SEMPRE, e a
   arbitragem de camada ocultava a leitura citada no mesmo render: a tela afirmava
   preservar o que ela mesma escondia (INV-7). A frase passa a ser função do MESMO
   veredito da arbitragem, calculado uma vez por render em `renderBlocks`.
   O default é falsy DE PROPÓSITO: sítio que esquecer de repassar o veredito deixa
   de afirmar a preservação — nunca passa a afirmá-la à toa. O papel nunca passa
   `true`, porque a Camada 1 nunca é impressa (spec §5/C13).
   A segunda oração ("nenhum produto é inferido sem contexto") é preservada em
   TODAS as variantes: é a promessa que o produto faz ao leitor, pinada por `V10`. */
function baseCardHTML(id, c, kind, afirmaPreservacao){
  const svcs = (c.services||[]).map(serviceHTML).join("");
  const mstate = esc32(MSTATE_LABELS[(c.maturity||{}).state]||"");
  const neutral = kind==="maturity"
    ? "Apoio baseado na maturidade — Landscape tecnológico não se aplica a esta capability."
    : afirmaPreservacao
      ? `Leitura V3.1.3 preservada (maturidade: ${mstate}). Informe o contexto desta capability para interpretação V3.2 — nenhum produto é inferido sem contexto.`
      : `Leitura de maturidade: ${mstate}. Informe o contexto desta capability para interpretação V3.2 — nenhum produto é inferido sem contexto.`;
  return `<div class="v32-card" data-cap="${escAttr(id)}">
    <div class="v32-card-head"><strong>${esc32(V32.CAPABILITIES[id].name)}</strong>
...
/* [010 · V1] "Há substituto?" — predicado PURO da spec §1, sobre `ctxRes.contexts`.
   Sem cache, sem campo, sem atributo DOM: é função da MESMA estrutura que a tela já
   consome no mesmo passe, logo nada é serializado (INV-8) e nenhum estado novo nasce.
   Três cláusulas:
     1. apresentação `card` — só quem vira card é candidato a substituir a leitura
        congelada; `base`/`maturity`/`null` nunca substituem nada;
     2. classificação ≠ CONTEXT_NOT_INFORMED — cláusula DEFENSIVA, não load-bearing:
        `CONTEXT_NOT_INFORMED` ⇒ supportMode "LEGACY-LABELLED", que `presentationOf`
        nunca promove a `card`, então a cláusula 1 já exclui o caso. Fica porque é ela
        que impediria um serviço anexado por `hasGap` sob `UNSET` de desligar a Camada 1
        sem substituir nada, se `presentationOf` um dia mudar (spec §1 e errata E1);
     3. payload não vazio — esta É a que carrega o peso: `presentationOf` devolve `card`
        INCONDICIONALMENTE para supportMode "VALIDATE", e uma capability em
        NEEDS_VALIDATION por resposta "não sei" produz card VAZIO, que não substitui
        recomendação nenhuma.
   O predicado é de SESSÃO — resíduo declarado em C3 do refinamento: sessão assimétrica
   oculta a Camada 1 e deixa a capability sem contexto muda na seção de apoio; o resíduo
   é fechado no card-alvo e sinalizado no bloco de ausência, não escondido. */
function hasSubstituteV32(ctxRes){
  const ctxs = (ctxRes && ctxRes.contexts) || {};
  return Object.keys(ctxs).some(id => {
    const c = ctxs[id];
    if (presentationOf(id, c) !== "card") return false;
    if (c.classification==="CONTEXT_NOT_INFORMED") return false;
...
/* [010 · V3] BLOCO DE AUSÊNCIA — helper ÚNICO para as DUAS superfícies (R9 §8), no
   ...
   capabilities afetadas: N cards que repetem a mesma não-informação são ruído; um
...
function baseAbsenceHTML(ids, ctxs, isScreen){
  ...
  const n = ids.length;
  const nomes = ids.map(id => esc32(V32.CAPABILITIES[id].name)).join("; ");
  const texto = `O contexto tecnológico não foi informado para ${n} ${n===1?"capability":"capabilities"}: ${nomes}. `
    + `Sem essa informação a interpretação V3.2 não é produzida para ${n===1?"ela":"elas"} — nenhum produto é inferido sem contexto.`;
  return isScreen
...
function renderCap(id, c, pres, afirmaPreservacao){
  ...
  return pres==="card" ? capCardHTML(id,c) : baseCardHTML(id,c,pres,afirmaPreservacao);
}
/* `afirmaPreservacao` = veredito da arbitragem do MESMO render (`!haSubstituto`),
   repassado por `renderBlocks`. Omitido ⇒ falsy ⇒ não afirma (spec §3). */
function buildSupportHTML(res, afirmaPreservacao){
...
/* O papel NUNCA afirma preservação: a Camada 1 não é impressa (spec §5/C13), então
   `afirmaPreservacao` fica falsy em todos os sítios de `buildPrintReport`. O parâmetro
   existe para que essa escolha seja explícita no contrato, e não implícita no esquecimento. */
function prCards(ids, ctxs, pres, afirmaPreservacao){
  return ids.map(id=>{
    const c = ctxs[id];
    const body = (pres==="card") ? capCardHTML(id,c) : baseCardHTML(id,c,pres===null?"maturity":pres,afirmaPreservacao);
...
function qsHowToReadHTML(){
  return `<div class="pr-howto" id="pr-howto">
    ...
    <ul class="pr-howto-list">
      ...
      <li>O relatório pode trazer <b>mais de uma lista</b> de possibilidades para o mesmo gap: elas partem de catálogos e ancoragens diferentes e <b>não se somam</b> como recomendação.</li>
    </ul></div>`;
}
...
/* [EA-66 · 2026-09-22] FALHA DE MONTAGEM NUNCA EMITE A TELA COMO RELATÓRIO.

   O DEFEITO QUE ISTO FECHA, medido em sessão real do proprietário: o PDF saiu
   com 13 folhas, doze quase em branco e a última com o anexo. Não era o
   relatório — era A TELA. `preparePrint()` estava pendurado CRU neste evento;
   qualquer exceção dentro dele deixava `v32-print-mode` sem entrar, e então
   `.wrap` continuava visível, `#v32-print-report` continuava `display:none`, e
   o navegador imprimia o workspace. A última página com o anexo é a assinatura
   do modo de falha: a regra congelada `#annex{break-before:page}` é da era em
   que imprimir ERA imprimir a tela, e hoje só se manifesta quando esta montagem
   falha.

   O custo desse silêncio é o pior possível neste produto: o artefato que vai ao
   cliente sai errado sem que ninguém seja avisado — nem o operador, que vê um
   diálogo de impressão normal.

   A guarda não conserta a causa; ela tira a falha do escuro. Reusa o estado
   `v32-print-blocked`, que já existe e já esconde `.wrap`: o resultado é UMA
   página dizendo o que houve, com a mensagem do erro para relato. Imprimir
   nada é recuperável; imprimir a tela achando que é o relatório, não. */
function v32PrintFallback(e){
  const el = ensurePrintContainer();
  while (el.firstChild) el.removeChild(el.firstChild);
  const box = document.createElement("div");
  box.className = "pr-blocked";
  box.setAttribute("data-pr-falha", "montagem");
  box.appendChild(document.createTextNode(
    "O relatório não pôde ser montado, e por isso nada foi impresso. " +
    "Cancele a impressão e relate esta mensagem: " + ((e && e.message) || String(e))));
  el.appendChild(box);
  document.body.classList.add("v32-print-blocked");
  if (window.console) console.error("V3.2 print: montagem falhou —", e);
}
window.addEventListener("beforeprint", function () {
  try { preparePrint(); } catch (e) { v32PrintFallback(e); }
});
...
window.__DEV = {
  ...
  legacySnapshot,
  ctx: () => lastCtx,
  /* [010 · V1] o predicado de arbitragem exposto como DADO DE TESTE (R-3: `__DEV` já é
     bridge registrado; nenhum `window.__*` novo, e `__V32UI` não cresce). Nenhum oráculo
     desta demanda o consome para julgar — quem lê o produto para julgar o produto não
     julga nada; `d010HasSubstitute` reimplementa a spec §1 na fixture. */
  hasSubstitute: res => hasSubstituteV32(res || lastCtx || V32.buildRecommendationContext()),
  qLabel,
